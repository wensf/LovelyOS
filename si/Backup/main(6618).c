...
void init(void)
{
	...
	printf("init test printf syscall\n");
...
void task_1(void)
{
	...
	printf("task_1 test printf syscall\n");
	...
	for (;;)
	{
		...
		if ( last_count != timer_c )
		{
			...
			int r = printf("pid=%d i=%08x,ticks: %08d utime=%8d ktime=%8d idle_cnt=%08d\n",
					current->pid,
					i,
					timer_c,
					get_utime(),
		 ...
		 if( x > 0 )
 		 ...
 		 draw_bitmap(x,  y,120*4,144*4,(unsigned char*)(0x800000));
 		 x++; x %= 1920;
